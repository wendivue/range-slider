...
| `type`       | string  | 'double' | Позволяет выбрать одиночное значение или интервальное   |
| `hasInputs`  | boolean | true     | Инпут(ы), в котором синхронизировано значения слайдера  |
| `hasRange`   | boolean | true     | Инпуты для минимальное значение и максимальное значение |
| `hasLabels`  | boolean | true     | Показывать элемент(ы) над бегунком                      |
| `hasScale`    | boolean | true     | Шкала значений                                          |
